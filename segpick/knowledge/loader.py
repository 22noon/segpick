...
def _text(value: Any, field: str, source: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ValueError(f"{source}: scenario field '{field}' must be non-empty text")
    return value.strip()

def _condition(raw: Any, source: str, sid: str) -> RuleCondition:
    if not isinstance(raw, dict):
        raise ValueError(f"{source}: scenario '{sid}' conditions must be mappings")
    kinds = [k for k in ("observation", "finding") if k in raw]
    if len(kinds) != 1:
        raise ValueError(f"{source}: scenario '{sid}' condition must define one observation or finding")
    kind = kinds[0]
...
def _conditions(raw: Any, source: str, sid: str) -> tuple[RuleCondition, ...]:
    ...
    if not isinstance(raw, list): raise ValueError(f"{source}: scenario '{sid}' conditions must be a list")
    return tuple(_condition(x, source, sid) for x in raw)

def load_knowledge_file(path: str | Path, *, source_label: str | None = None) -> tuple[KnowledgeModule, ...]:
...
def load_active_scenarios(user_files: Iterable[str | Path] = ()) -> tuple[tuple[KnowledgeModule, ...], tuple[KnowledgeModule, ...]]:
    builtin = load_knowledge_file(Path(__file__).with_name("default_scenarios.yml"), source_label="builtin:default_scenarios.yml")
