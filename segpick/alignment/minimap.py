...
def run_minimap(anchor_fasta: str | Path, query_fasta: str | Path, paf_out: str | Path, preset: str = "asm5", cigar: bool = True) -> Path:
    """Run minimap2 and write a PAF file."""
    if shutil.which("minimap2") is None: raise RuntimeError("minimap2 was not found on PATH")
    paf_out = Path(paf_out); paf_out.parent.mkdir(parents=True, exist_ok=True)
    cmd = ["minimap2", "-x", preset]
    if cigar: cmd.append("-c")
    cmd.extend([str(anchor_fasta), str(query_fasta)])
    with paf_out.open("w") as handle: subprocess.run(cmd, stdout=handle, check=True)
    return paf_out
...
def align_gene(gene: Gene, anchor_fasta: str | Path, query_fasta: str | Path, paf_out: str | Path, preset: str = "asm5") -> Gene:
