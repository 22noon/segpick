...
def summarise_alignments(alignments):
    ...
    tlen = alignments[0].target_length
    ...
    matches = sum(a.matches for a in alignments)
    alen = sum(a.alignment_length for a in alignments)
    largest = max((a.query_end - a.query_start for a in alignments), default=0)
    qcov = aq / qlen if qlen else 0
    tcov = at / tlen if tlen else 0
    ident = matches / alen if alen else 0
    frag = 1 - largest / aq if aq else 1
    left = min(a.query_start for a in alignments)
    right = max(0, qlen - max(a.query_end for a in alignments))
    strands = {a.strand for a in alignments}
    orient = next(iter(strands)) if len(strands) == 1 else "mixed"
    ...
    return ContainmentMetrics(
        ...
        orient,
        score,
        classify_status(qcov, ident, frag, len(alignments)),
    )


def analyse_gene(gene):
    byq = defaultdict(list)
    for a in gene.alignments:
        byq[a.query_id].append(a)
    for c in gene.candidates:
        if c.id == gene.anchor_id:
            c.analysis.containment = ContainmentMetrics(
                c.length,
                c.length,
                c.length,
                c.length,
    ...
    for r in gene.references:
        if r.accession == gene.anchor_id:
            r.containment = ContainmentMetrics(
                r.length,
                r.length,
                r.length,
                r.length,
