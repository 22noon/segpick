...
from segpick.config import load_config, resolve_config, RunConfig
from segpick.provenance import write_provenance
from segpick.io.builder import build_sample
from segpick.alignment.export import export_gene_fastas, export_anchor_fastas, safe_name
...
def _add_run_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--use-existing-paf", action=argparse.BooleanOptionalAction, default=None
    )
...
def _cli_override_values(args: argparse.Namespace) -> dict[str, object]:
    keys = (
        "hits", "contigs", "refs", "outdir", "sample_name", "strict",
        "align", "use_existing_paf", "preset", "html",
    )
