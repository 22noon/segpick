...
class Gene:
    ...
    def add_candidate(self, contig: CandidateContig) -> None: self.candidates.append(contig)
    def add_reference(self, reference: ReferenceSequence) -> None:
        if reference.accession not in {r.accession for r in self.references}: self.references.append(reference)
    def all_records(self) -> list[SeqRecord]: return [c.record for c in self.candidates] + [r.record for r in self.references]
    ...
    def anchor_record(self) -> SeqRecord | None:
        ...
        for c in self.candidates:
            if c.id == anchor: return c.record
        for r in self.references:
            if r.accession == anchor: return r.record
        return None
    ...
    def attach_alignments(self, alignments: list[Alignment]) -> None:
        ...
        by_id = {c.id: c for c in self.candidates}; by_id.update({r.accession: r for r in self.references})
        for aln in alignments:
            obj = by_id.get(aln.query_id)
            if obj is not None: obj.alignments.append(aln)
    def summary_dict(self) -> dict[str, object]:
        best = self.best_by_confidence()
        return {"gene": self.name, "segment": self.segment, "n_candidates": len(self.candidates), "n_references": len(self.references), "n_alignments": len(self.alignments), "anchor": self.anchor_id or self.longest_sequence_id(), "best_by_confidence": best.id if best else None}
