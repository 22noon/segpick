...
Given an ObservationNode, virtually remove it and re-evaluate
...
class CounterfactualResult:
    ...
    removed_node_id: str
    removed_node_type: Literal["observation"]
...
def _build_counterfactual_context(
    ...
    # For the test, we need to create pattern definitions from the candidate's patterns
    # Map pattern_id -> (requires, supports, conflicts) from the pattern definitions
    ...
    candidate_pattern_ids = {p.pattern_id for p in analysis.evidence_patterns}
    filtered_pattern_defs = tuple(d for d in all_pattern_defs if d.pattern_id in candidate_pattern_ids)
    
    # Load all hypothesis definitions from knowledge base
    from segpick.knowledge import load_active_hypotheses
    all_hyp_defs, _ = load_active_hypotheses()
    
    # Filter to only those hypotheses present in the candidate's analysis
    candidate_hyp_ids = {h.hypothesis_id for h in analysis.biological_hypothesis_evaluations}
    filtered_hyp_defs = tuple(d for d in all_hyp_defs if d.hypothesis_id in candidate_hyp_ids)
    ...
    return _CounterfactualContext(
        ...
        pattern_definitions=filtered_pattern_defs,
        hypothesis_definitions=filtered_hyp_defs,
...
def evaluate_counterfactual(
    ...
    Args:
        ...
        node_id: ID of the ObservationNode to virtually remove (must start with "observation:")
    ...
    # Verify this is an ObservationNode
    if not node_id.startswith("observation:"):
        raise ValueError(f"Counterfactual evaluation currently only supports ObservationNode removal (got '{node_id}')")
    
    # Check if node exists in graph
    all_observation_ids = {n.id for n in context.original_reasoning_graph.observations}
    if node_id not in all_observation_ids:
        raise KeyError(f"Unknown observation node '{node_id}'")
    
    # Use existing Impact traversal to find affected hypotheses
    explorer = ReasoningExplorer(context.original_reasoning_graph)
    impact = explorer.impact(node_id)
    
    # Identify affected patterns and hypotheses from Impact paths
    affected_pattern_ids = set()
    affected_hypothesis_ids = set()
    for path in impact.paths:
        for node in path.nodes:
            if node.id.startswith("pattern:"):
                affected_pattern_ids.add(node.id)
            elif node.id.startswith("hypothesis:"):
                affected_hypothesis_ids.add(node.id)
    
    # Create counterfactual context (virtually remove the observation)
    cf_context = _remove_observation(context, node_id)
    
    # Re-evaluate affected patterns
